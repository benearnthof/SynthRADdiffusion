...
# Instead of text conditioning we use an auxiliary net to embed the MRI data
# so we can condition the generation of CT images on their respective MRI
# counterparts
...
from synthraddiffusion.ddpm.text import (
    tokenize,
    bert_embed,
    BERT_MODEL_DIM,
)  # TODO adjust for auxiliary embedding net
...
# we need a way to encode the position of every frame relative to the other frames
...
class RelativePositionBias(nn.Module):
    ...
    def __init__(
        ...
        heads=8,
        num_buckets=32,
        max_distance=128,  # seems reasonable given the resolution of the data
    ):
        super().__init__()
...
class EMA:
    """
    Exponential moving average helper class to keep an EMA copy of the model during training.
    Is invoked every x iterations and returns weighted average of model parameters obtained during training.
    """
...
class Residual(nn.Module):
    """
    Wrapper for Residual layer. Initialize with function `fn`.
    """
...
class SinusoidalPosEmb(nn.Module):
    """
    Sinusoidal Positional Embedding proposed in the Attention is All You Need paper.
    """
...
def Upsample(dim):
    """
    Upsample Wrapper.
    We expect single channel images so dim will be 1.
    Batch, Channels, Depth, Height, Width -> Batch, Channels, Depth, Height*2, Width*2
    """
...
class LayerNorm(nn.Module):
    """Layer Normalization with learnable scale parameter but no bias term."""

    def __init__(self, dim, eps=1e-5):
        super().__init__()
        self.eps = eps
        self.gamma = nn.Parameter(torch.ones(1, dim, 1, 1, 1))  # learnable scale
...
class PreNorm(nn.Module):
    """Pre Normalization for Transformers, more stable than Post Norm"""
...
class Block(nn.Module):
    """
    Main Building Block for Resnet.
    Every separate block consists of a 3d convolution, a normalization, and a nonlinearity
    """

    def __init__(self, dim, dim_out, groups=8):
        super().__init__()
        # space only convolution
        self.proj = nn.Conv3d(dim, dim_out, (1, 3, 3), padding=(0, 1, 1))
        # https://arxiv.org/abs/1803.08494 Group Norm
...
class ResnetBlock(nn.Module):
    """
    Resnet Block with MLP for Time Embedding
    Resnet performs space only convolution (1, 3, 3), treats first axis as a batch axis.
    This space-time factorized approach makes it possible to jointly train on images & videos.
    """

    def __init__(self, dim, dim_out, *, time_emb_dim=None, groups=8):
        super().__init__()
        # TODO: Experiment with different MLP sizes.
        # Increasing the size of the conditioning net should improve performance
        self.mlp = (
    ...
    def forward(self, x, time_emb=None):
        scale_shift = None
        if exists(self.mlp):
            # time embedding to extend Unet to 3 dimensions
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        if vqgan_ckpt:  # VQGAN for diffusion input. #TODO: Add VQGAN for conditioning
